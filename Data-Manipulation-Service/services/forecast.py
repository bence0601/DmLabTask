...
class Forecast:
    ...
    def forecast_logic(city_id, city):
        ...
        temp_forecast = model_temp.predict(
            [[len(dates)], [len(dates) + 1], [len(dates) + 2]]
        )

        y_wind = winds
        model_wind = LinearRegression().fit(X, y_wind)
        wind_forecast = model_wind.predict(
            [[len(dates)], [len(dates) + 1], [len(dates) + 2]]
        )

        y_precip = precips
        model_precip = LinearRegression().fit(X, y_precip)
        precip_forecast = model_precip.predict(
            [[len(dates)], [len(dates) + 1], [len(dates) + 2]]
        )
    ...
    def create_forecast_for_city(city):
        ...
        if existing_id is None:
            print(
                f"A(z) {city} város nincs az adatbázisban. Új adatokat fogunk lekérni."
            )
            city_data_from_api = Forecast.fetch_weather_data_if_needed(city)

            if not city_data_from_api:
                print(
                    "Nem lehet megjósolni az időjárást a kért városra, mert erre a városra nincsenek adatok a központi db-ben"
                )
                return None
            else:
